...
const MissionRequestForm: FC<MissionRequestFormProps> = ({ user, users, onSubmit, onDirectOrder, onCancel, initialData, isDarkMode }) => {
    ...
    const handleSubmit = (e: FormEvent, isDraft: boolean = false) => {
        ...
        if (!isDraft && formData.tipo_missao === 'SOBREAVISO' && onDirectOrder) {
        ...
        <form onSubmit={(e) => handleSubmit(e, false)} className={`glass-panel w-full h-full sm:h-auto sm:rounded-2xl shadow-xl overflow-hidden sm:max-h-[90vh] flex flex-col`}>
                                ...
                                onChange={e => {
                                    ...
                                    if (val === 'SOBREAVISO') {
                                        const directData = {
                                            ...
                                            dados_missao: {
                                                ...formData,
                                                tipo_missao: 'SOBREAVISO',
                                                local: 'GSD-SP',
                                                informacoes_complementares: 'SOBREAVISO DIÁRIO'
                                            }
                                        };
                                        onDirectOrder?.(directData);
                                    }
                                }}
            ...
            <div className={`p-4 sm:p-6 border-t glass-panel flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 shrink-0`}>
                <button
                    ...
                    onClick={(e) => {
                        e.preventDefault();
                        if (formData.tipo_missao === 'SOBREAVISO') {
                ...
                >
                    ...
                    {formData.tipo_missao === 'SOBREAVISO' ? 'Definir Efetivo →' : 'Enviar Solicitação'}
                </button>
                <button
                    type="button"
