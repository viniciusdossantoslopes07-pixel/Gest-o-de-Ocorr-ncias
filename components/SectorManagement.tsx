...
import { Plus, Trash2, Pencil, Settings2, AlertTriangle, X, CheckCircle, Loader2, GripVertical } from 'lucide-react';
...
const SectorManagement: FC<SectorManagementProps> = ({ currentUser, isDarkMode = false, users }) => {
    const { sectors, oms, addSector, removeSector, renameSector, reorderSectors, loading, omId } = useSectors();
    ...
    const handleSaveRename = async (id: string) => {
        ...
        const { error } = await renameSector(id, editingName);
        setIsSavingEdit(false);
        if (error) {
            alert(`Erro ao renomear setor: ${error}`);
        } else {
            setEditingId(null);
            setEditingName('');
        }
    ...
    const renderSectorItem = (sector: any) => {
        ...
        return (
            <div
                ...
                className={`flex items-center justify-between px-6 py-4 transition-colors ${!isEditing ? 'cursor-move' : ''} 
                        ...
                        {isEditing ? (
                            <div className="flex items-center gap-2 max-w-sm">
                                <input
                                    ...
                                    onKeyDown={(e) => {
                                        if (e.key === 'Enter') handleSaveRename(sector.id);
                                        if (e.key === 'Escape') setEditingId(null);
                                    }}
                                    autoFocus
                                    maxLength={30}
                                    className={`w-full rounded-xl px-3 py-1.5 text-xs font-black border focus:ring-2 focus:ring-blue-500 outline-none uppercase transition-all ${
                                        dk ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300 text-slate-900'
                                    }`}
                                />
                                <button
                                    onClick={() => handleSaveRename(sector.id)}
                                    disabled={isSavingEdit}
                                    className="p-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 transition-all flex items-center justify-center"
                                    title="Salvar alterações"
                                >
                                    {isSavingEdit ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle className="w-3.5 h-3.5" />}
                                </button>
                                <button
                                    onClick={() => setEditingId(null)}
                                    disabled={isSavingEdit}
                                    className={`p-2 rounded-xl transition-all ${dk ? 'text-slate-400 hover:text-white hover:bg-slate-800' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`}
                                    title="Cancelar"
                                >
                    ...
                    {!isEditing && (!isConfirming ? (
                        <>
                            <button
                                ...
                                disabled={isDeleting}
                                className={`p-2 rounded-xl transition-all ${dk ? 'text-slate-400 hover:text-blue-400 hover:bg-blue-900/20' : 'text-slate-400 hover:text-blue-600 hover:bg-blue-50'}`}
