...
const UserStatistics: React.FC<UserStatisticsProps> = ({ users, isDarkMode }) => {
    const { sectors } = useSectors();

    // Filtra apenas o pessoal real ativo
    const activeUsers = useMemo(() => users.filter(u => u.active !== false && !u.is_functional), [users]);

    // Estatísticas por Posto/Graduação (seguindo a ordem de RANKS)
    ...
    // Estatísticas por Unidade (GSD-SP vs BASP)
    const unitStats = useMemo(() => {
        ...
        activeUsers.forEach(u => {
            ...
            else gsd++; // Default GSD-SP se não encontrado
        });
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Efetivo por Unidade */}
                <div className={`p-6 md:p-8 rounded-[2rem] border shadow-sm ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
                    <div className="flex items-center gap-3 mb-6">
                        <div className={`p-2.5 rounded-xl ${isDarkMode ? 'bg-indigo-500/20 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}>
                            <PieChartIcon className="w-5 h-5" />
                        </div>
                        <h3 className={`text-sm font-black uppercase tracking-widest ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Distribuição por Unidade</h3>
                    </div>
                ...
                <div className={`p-6 md:p-8 rounded-[2rem] border shadow-sm ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className={`text-sm font-black uppercase tracking-widest ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Maiores Setores</h3>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {topSectors.length === 0 && (
                            <p className="text-center text-slate-500 text-xs py-4">Nenhum dado de setor.</p>
                        )}
                    </div>
                </div>
            </div>
            ...
            <div className={`p-6 md:p-8 rounded-[2rem] border shadow-sm ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
                ...
                <div className="h-64 flex items-end gap-2 md:gap-4 pb-4 overflow-x-auto custom-scrollbar px-2">
                    {rankStats.map((stat) => (
                    ...
                    {rankStats.length === 0 && (
                        <p className="w-full text-center text-slate-500 text-xs py-10">Nenhum dado cadastrado.</p>
                    )}
                </div>
            </div>
        </div>
