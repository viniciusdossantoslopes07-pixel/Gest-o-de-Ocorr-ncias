...
export const OmPrintHeader: React.FC<OmPrintHeaderProps> = ({ om, className = "" }) => {
    ...
    const omLogo = om?.logo_url || (isGsdSp ? "/logo_gsd.png" : (isBasp ? "/logo_basp_optimized.png" : null));
    const hostLogo = om?.host_logo_url || (isGsdSp || isBasp ? "/logo_basp_optimized.png" : null);
        ...
        <div className={`flex items-start justify-between mb-4 border-b-2 border-slate-900 pb-3 ${className}`}>
            ...
            <div className="flex-1 text-center px-2 flex flex-col justify-center">
                ...
                <h2 className="text-[11px] sm:text-sm font-black uppercase text-slate-800 tracking-wide mt-1">
                    {om ? om.host_unit : "BASE AÉREA DE SÃO PAULO"}
                </h2>
                <h3 className="text-[9px] sm:text-xs font-bold uppercase text-slate-700 leading-tight">
                    {om ? (om.name || '') : "GRUPO DE SEGURANÇA E DEFESA DE SÃO PAULO"}
                    {om?.acronym && !om?.name?.includes(om.acronym) ? ` (${om.acronym})` : (!om ? " (GSD-SP)" : "")}
                </h3>
            </div>
