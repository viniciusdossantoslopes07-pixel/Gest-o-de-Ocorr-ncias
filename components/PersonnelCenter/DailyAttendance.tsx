...
const SectorPicker: FC<{
    ...
    return (
        <div ref={containerRef} className="relative">
            <button
                onClick={toggleOpen}
                className={`p-1.5 rounded-lg transition-all ${isOpen ? 'bg-indigo-500 text-white' : 'hover:bg-indigo-500/10 text-slate-500 hover:text-indigo-500'}`}
                            ...
                            {DISPLAY_SECTORS.filter(s => s.trim() !== currentSector.trim()).map(s => (
                                <button
                                    ...
                                    className={`text-left px-4 py-3 rounded-xl text-[11px] font-bold transition-all border border-transparent ${isDarkMode
                                        ...
                                        : 'hover:bg-indigo-50 text-indigo-900 hover:border-indigo-100'
                                        }`}
                                >
                                    {s}
                                </button>
